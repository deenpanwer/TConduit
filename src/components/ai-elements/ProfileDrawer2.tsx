...
const ProfileDrawer2 = ({ isOpen, onClose, profile }: ProfileDrawer2Props) => {
  ...
  const readme = isFlat ? (profile?.readme_text || "") : (profile?.readme || "");
  ...
  const languages = isFlat ? (profile?.top_languages || []) : Array.from(new Set(repos.map((r: any) => r.language).filter(Boolean))).slice(0, 5);
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-[85vw] md:max-w-[75vw] lg:max-w-[65vw] flex flex-col border-l dark:border-slate-800 p-0">
        <SheetHeader className="sr-only">
          <SheetTitle>{user.name || user.login || "Candidate Profile"}</SheetTitle>
          <SheetDescription>Detailed technical profile and activity history</SheetDescription>
        </SheetHeader>
            ...
            <div className="p-8 border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              <div className="flex flex-col md:flex-row gap-6 items-start">
                <Avatar className="h-24 w-24 rounded-[2rem] shadow-xl ring-4 ring-white dark:ring-slate-950">
                  ...
                  <AvatarFallback className="text-2xl">{user.login?.charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
                
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    <h2 className="text-3xl font-black tracking-tighter">{user.name || user.login}</h2>
                    {user.hireable && (
                  ...
                  <p className="text-blue-600 dark:text-blue-400 font-bold uppercase tracking-widest text-sm">@{user.login}</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {[
                      ...
                      { label: "Public Gists", value: user.public_gists, icon: FileText, color: "text-orange-500" },
