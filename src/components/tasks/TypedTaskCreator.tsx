...
import { useTasks, Task, Priority, Subtask, Draft } from "@/hooks/useTasks";
...
interface TypedTaskCreatorProps {
  editingNewTask: Draft | Partial<Task> | null | undefined;
  onUpdateTask: (
    id: string,
  ...
  return (
    <div className="space-y-4">
      <label className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest flex items-center gap-2">
        <MessageSquare size={12} className="text-primary" /> Comments (
        {(comments || []).length})
      </label>
      ...
      <div className="flex items-start gap-3 bg-secondary/20 p-2 rounded-xl border border-border/40 focus-within:border-primary/40 transition-all">
        ...
        <Button
          ...
          onClick={async () => {
            if (taskId === "new") {
              toast.error("Finish creating the task first to add comments.");
              return;
            }
...
export function TypedTaskCreator({
  ...
  const [showBottomFadeDescription, setShowBottomFadeDescription] =
    useState(false);
    ...
    if (file.size > MAX_SIZE) {
      toast.error(`File exceeds ${isPremium ? "250MB" : "10MB"} limit.`, {
        description: !isPremium
          ? "Upgrade to Premium for 250MB file support."
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (
          shouldSaveRecordingRef.current &&
  ...
  const { deadlineValue, deadlineUnit, noDeadline } = useMemo(() => {
    ...
    if (hours > 0 && hours % (30 * 24) === 0) {
      return {
        noDeadline: false,
        deadlineValue: hours / (30 * 24),
        deadlineUnit: "months" as const,
      };
    }
    if (hours > 0 && hours % 24 === 0) {
      return {
        noDeadline: false,
        deadlineValue: hours / 24,
    ...
    <motion.div
      ...
      className={cn(
        ...
        isMobile
          ...
          : "inset-y-4 right-4 w-1/2 rounded-2xl",
      )}
    >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="secondary"
                size="sm"
                className="h-8 px-3 rounded-full bg-background/50 backdrop-blur-md hover:bg-background border shadow-sm flex items-center gap-2"
              >
                ...
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      onUpdateTask(
                        "new",
                        { coverImage: e.currentTarget.value },
                        "cover_image_updated",
                      );
                    }
                  }}
            ...
            <DropdownMenu>
                ...
                >
                  <div
                    className={cn(
                      ...
                      PRIORITIES[editingNewTask?.priority || "medium"].color,
                    )}
                  />
              ...
              <PopoverTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"
                >
                  <Calendar size={12} className="mr-2 h-4 w-4" />
                  {editingNewTask?.dueDate
                    ? format(new Date(editingNewTask.dueDate), "MMM d")
                    : "Set Date"}
                </Button>
              </PopoverTrigger>
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className={cn(
                ...
                (editingNewTask?.title?.length || 0) > 5 ||
                  (editingNewTask?.description?.length || 0) > 10 ||
                  (editingNewTask?.voiceNotes &&
                    editingNewTask.voiceNotes.length > 0)
            ...
            >
              {isEnhancing ? (
                <Wand2 className="size-3 animate-spin" />
              ) : (
            ...
            <Button
              variant="ghost"
              className={cn(
                "h-8 px-3 rounded-md text-xs flex items-center gap-2 transition-colors",
                isBulkMode
          ...
          <>
            <div
              className={cn("relative w-full", {
                "fade-top": showTopFadeTitle,
                "fade-bottom": showBottomFadeTitle,
              })}
            >
              {isEnhancing ? (
              ...
              <label className="flex items-center justify-between mb-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    ...
                    <DropdownMenuSeparator />
                    {personnel.map((p: any) => {
                      const isAssigned = (editingNewTask?.assignees || []).some(
                        (uid: any) => uid === p.id,
                      );
                      return (
                        <DropdownMenuItem
              ...
              <div className="flex flex-wrap gap-2">
                {(editingNewTask?.assignees || []).length > 0 ? (
                  editingNewTask?.assignees?.map((uid: any) => {
                    ...
                    return (
                      <Badge
                        key={`new-task-assignee-${uid}`}
                        variant="secondary"
                        className="pl-1 pr-2 py-1 gap-2 hover:bg-secondary/80"
                      >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1 bg-secondary/20 p-1 rounded-lg border border-border/50">
                  <Button
                    ...
                    onClick={() =>
                      onUpdateTask("new", {
                        leaderPoints: Math.max(
                          0,
                          (editingNewTask?.leaderPoints || 0) - 10,
                        ),
                      })
                    }
                  >
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      onUpdateTask("new", {
                        leaderPoints: Math.max(0, Number(e.target.value)),
                      })
                    }
                  />
                  <Button
                    ...
                    onClick={() =>
                      onUpdateTask("new", {
                        leaderPoints: (editingNewTask?.leaderPoints || 0) + 10,
                      })
                    }
                  >
                ...
                <AnimatePresence>
                  {!noDeadline && (
                    ...
                    >
                      ...
                      <Select
                        value={deadlineUnit}
                        onValueChange={(u: "hours" | "days" | "months") =>
                          handleDeadlineChange(deadlineValue, u, false)
                        }
                      >
            ...
            <div className="mb-8 group">
              ...
              <div
                className={cn("relative w-full rounded-xl", {
                  "fade-top": showTopFadeDescription,
                  "fade-bottom": showBottomFadeDescription,
                })}
              >
                {isEnhancing ? (
            ...
            {(editingNewTask as any)?.id &&
              (editingNewTask as any)?.id !== "new" && (
                ...
                <div className="flex gap-0.5 items-center">
                  {[...Array(20)].map((_: any, i: number) => (
                    <motion.div
                      key={i}
                      animate={{
                        height: isPaused ? 2 : Math.random() * 20 + 2,
                      }}
                      transition={{
                        repeat: Infinity,
                        duration: 0.2,
            ...
            >
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  onClick={onCancel}
                  className="h-10 px-6 rounded-lg text-xs font-black uppercase tracking-widest border-2 border-border/50 hover:bg-secondary/50 transition-all"
                >
                  Cancel
                </Button>
                <Button
                  onClick={onSave}
                  className="h-10 px-6 rounded-lg text-xs font-black uppercase tracking-widest shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all"
                  disabled={
                    isEnhancing || (!isBulkMode && !editingNewTask?.title)
                  }
                >
                  Done
                </Button>
              ...
              <motion.button
                ...
                transition={
                  isRecording ? { repeat: Infinity, duration: 1.5 } : {}
                }
                className={cn(
                  "h-12 w-12 rounded-full flex items-center justify-center transition-all shadow-lg",
                  isRecording
                    ? "bg-red-500 text-white shadow-red-500/20"
                    : "bg-secondary/30 hover:bg-primary hover:text-primary-foreground",
                )}
                onClick={
