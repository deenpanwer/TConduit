...
export function ContactModal({ isOpen, onOpenChange, mode: initialMode, contact, initialStage, onClose }: ContactModalProps) {
  const { addEntity, updateEntity, config, updateConfig } = useCRMContacts();
  ...
  useEffect(() => {
    ...
    if (isOpen) {
        if ((initialMode === 'edit' || initialMode === 'preview') && contact) {
            setFormData({ ...contact.data, name: contact.name });
  ...
  }, [isOpen, initialMode, contact, initialStage, config.fields]);
  ...
  const handleSubmit = async () => {
    ...
    } else if (currentMode === 'edit' && contact) {
      await updateEntity(contact.id, { ...entityData, name: contactName });
    }
    handleClose();
  };
  ...
  const getTitle = () => {
      switch(currentMode) {
          ...
          case 'preview': return 'Contact Profile';
          case 'configure': return 'Configure Contact Fields';
          default: return '';
      }
  }
          ...
          <div className='flex items-center gap-4'>
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 text-[8px] font-black uppercase tracking-widest border border-blue-500/20">Contact Intelligence</span>
                </div>
          ...
          {currentMode === 'configure' ? (
            ...
            <>
                ...
                {currentMode !== 'preview' && (
                    ...
                    className="rounded-xl font-black text-[10px] uppercase tracking-widest h-12 px-10 bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
                    >
                    {currentMode === 'create' ? 'Launch Contact' : 'Save Changes'}
                    </Button>
                )}
            </>
          )}
