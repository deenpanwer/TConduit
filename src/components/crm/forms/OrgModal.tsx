...
export function OrgModal({ isOpen, onOpenChange, mode: initialMode, organization, initialStage, onClose }: OrgModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    const organizationName = entityData.name || `Unnamed Organization ${new Date().getTime()}`;

    if (currentMode === 'create') {
      await addEntity({ name: organizationName, data: entityData });
    } else if (currentMode === 'edit' && organization) {
      await updateEntity(organization.id, { ...entityData, name: organizationName });
    }
    handleClose();
  };
  ...
  const handleConfigSave = async () => {
    ...
    console.log("New organization field configuration saved!");
          ...
          <div className='flex items-center gap-4'>
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 text-[8px] font-black uppercase tracking-widest border border-blue-500/20">Organization Intelligence</span>
                </div>
