...
interface MobileInvoiceCanvasProps {
    invoice: InvoiceData;
    subtotal: number;
    brandingLogo: string | null;
}

export function MobileInvoiceCanvas({ invoice, subtotal, brandingLogo }: MobileInvoiceCanvasProps) {
        ...
        <div className="flex flex-col gap-6 items-center w-full">
            {pages.map((pageItems, pageIdx) => (
                <div 
                    ...
                    className="relative overflow-hidden shrink-0 shadow-lg rounded-sm border border-slate-200 dark:border-slate-800"
                >
                    <div 
                        className="invoice-page w-[800px] h-[1131px] bg-white dark:bg-slate-950 p-12 md:p-16 flex flex-col text-slate-800 dark:text-slate-200 absolute top-0 left-0 origin-top-left shrink-0"
                        style={{
                                ...
                                <div className="flex justify-between gap-12">
                                    ...
                                    <div className="w-56 space-y-8 text-right">
                                        <div className="space-y-3 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                                            ...
                                            <div className="flex justify-between text-lg font-black uppercase border-t border-slate-200 dark:border-slate-700 pt-3 text-blue-600">
                                                <span>Total</span>
                                                <span>{invoice.currency} {subtotal.toLocaleString()}</span>
                                            </div>
                                        </div>
                                        ...
                                        {!invoice.hiddenFields?.['signature'] && (
                                            <div className="pt-6">
                                                <div className="inline-block border-b-2 border-slate-900 dark:border-white min-w-[180px] text-center pb-1">
                                                    <p className="text-xl font-signature italic tracking-[0.1em] text-slate-900 dark:text-white" style={{ fontFamily: 'Dancing Script, cursive' }}>
                                                        {invoice.signature || ''}
                                                    </p>
                                                </div>
