...
import { 
  ...
  Layers, CheckCircle2, ImageIcon, Sparkles, ChevronRight, Zap
} from "lucide-react";
...
export function EmployeeAnomalyModal({ report, onClose, onNavigateToScreenshots }: EmployeeAnomalyModalProps) {
  const [selectedInstanceIndex, setSelectedInstanceIndex] = useState<number>(0);
  const [excusing, setExcusing] = useState(false);

  if (!report) return null;

  const flags = report.flags && report.flags.length > 0 ? report.flags : [report.flag];
  const activeFlag = flags[selectedInstanceIndex] || flags[0];
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-4 rounded-xl border border-gray-100 bg-slate-50/50 space-y-1">
                      ...
                      <p className="text-sm font-bold text-slate-800 font-mono">
                        {activeFlag.timeWindow || activeFlag.metric}
                      </p>
                    </div>
