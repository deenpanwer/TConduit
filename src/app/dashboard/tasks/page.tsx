        ...
        {selectedTask && (
          <>
            <motion.div
              ...
              exit={{ opacity: 0 }}
              onClick={() => {
                  if (selectedTaskId === "new") setEditingNewTask(null);
                  setSelectedTaskId(null);
              }}
              className="fixed inset-0 z-40 bg-background/80 backdrop-blur-sm"
            />
            ...
            >
                      ...
                      <PopoverContent className="w-80 p-3">
                         <div className="space-y-2">
                            ...
                            <Input 
                               ...
                               onKeyDown={(e) => {
                                  if(e.key === 'Enter') {
                                     handleUpdateTaskLocal(selectedTaskId!, { coverImage: e.currentTarget.value });
                                  }
                               }}
                            />
                         </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 -mt-6 relative bg-card rounded-t-3xl border-t border-border/50 custom-scrollbar">
                 ...
                 <div className="flex items-center gap-3 mb-6">
                    <DropdownMenu>
                       <DropdownMenuTrigger asChild disabled={!canManageTasks && selectedTaskId !== 'new'}>
                          <Button variant="outline" size="sm" className="h-7 text-[10px] font-bold uppercase tracking-wider gap-2 border-border/50" disabled={!canManageTasks && selectedTaskId !== 'new'}>
                             <div className={cn("w-2 h-2 rounded-full", PRIORITIES[selectedTask.priority || 'medium'].color)} />
                             {selectedTask.priority || 'Medium'} Priority
                          </Button>
                       </DropdownMenuTrigger>
                       { (canManageTasks || selectedTaskId === 'new') && (
                    ...
                    <Popover>
                      <PopoverTrigger asChild disabled={!canManageTasks && selectedTaskId !== 'new'}>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] font-bold uppercase tracking-wider text-muted-foreground" disabled={!canManageTasks && selectedTaskId !== 'new'}>
                 ...
                 <div className={cn("relative w-full",
                    ...
                    <AutoResizingTextarea
                       ...
                       readOnly={!canManageTasks && selectedTaskId !== 'new'}
                    />
                 </div>
              ...
              <div className="flex items-center justify-end gap-2 p-4 border-t border-border/50 bg-card shrink-0">
                 ...
                 <Button onClick={() => {
                    if (selectedTaskId === "new") {
                       handleSaveNewTask();
                    } else if (localTask && originalTask) {
