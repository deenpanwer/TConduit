...
import { 
    Printer, ArrowLeft, CheckCircle2, UserCheck, 
    Receipt, Utensils, Info, CreditCard 
} from 'lucide-react';
...
const InvoiceDetails = ({
    ...
    const customer = customerId ? customers.find((c: any) => c.id === customerId) : null;
    const saleItems = data.items || [];

    type SaleItem = typeof saleItems[number];

    const subtotal = useMemo(() => saleItems.reduce((sum: number, item: SaleItem) => sum + (item.lineItemTotal || 0), 0), [saleItems]);
    const discountAmount = useMemo(() => saleItems.reduce((sum: number, item: SaleItem) => sum + (item.quantity * (item.unitPrice || 0) * ((item.discount || 0) / 100)), 0), [saleItems]);
    ...
    const grandTotal = useMemo(() => taxableAmount + taxAmount, [taxableAmount, taxRate]);


    const saleCreatedAt = data.createdAt;
    const saleId = data.id;
    const cashierName = isSale ? (data as SaleTransaction).cashierName : (data as PosTicket).createdBy?.name;
    const tableId = (data as PosTicket).tableId || (isSale ? (data as SaleTransaction).tableId : null);
    const table = tableId ? tables.find((t: any) => t.id === tableId) : null;
                        ...
                        {!customer && (
                          <details className="mt-4">
                            ...
                            <form onSubmit={handleQuickCustomerAdd} className="space-y-4 pt-4 border-t mt-2">
                                <Input placeholder="Customer Name*" value={quickCustomer.name} onChange={(e: any) => setQuickCustomer({...quickCustomer, name: e.target.value})} />
                                <Input placeholder="Phone Number (Optional)" value={quickCustomer.phone} onChange={(e: any) => setQuickCustomer({...quickCustomer, phone: e.target.value})} />
...
export default function InvoicePage() {
  ...
  useEffect(() => {
    const fetchEntity = async () => {
      const result = await pos.getEntityForInvoice(invoiceId);
      setEntityData(result);
    };
    if (invoiceId) fetchEntity();
  }, [invoiceId, pos.getEntityForInvoice]);
  ...
  const handleQuickCustomerAdd = async (e: React.FormEvent) => {
    ...
    try {
        const newCustomerId = await pos.addCustomer({ name: quickCustomer.name, phoneNumber: quickCustomer.phone });
        if (newCustomerId && entityData && entityData.type === 'ticket' && entityData.data) {
            await pos.linkCustomerToTicket(entityData.data.id, newCustomerId);
            const result = await pos.getEntityForInvoice(invoiceId); // Refresh data
            setEntityData(result);
  ...
  const handleProceedToPayment = () => {
    if (entityData && entityData.type === 'ticket' && (entityData.data as PosTicket).tableId) {
      pos.selectTable((entityData.data as PosTicket).tableId as string); // Set the correct table context
  ...
  if (!entityData.data || entityData.type === 'notFound') {
    return <NotFoundScreen router={router} />;
  }
