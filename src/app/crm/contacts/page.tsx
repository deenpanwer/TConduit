...
export default function ContactsPage() {
  ...
  const handleCreateContact = () => {
    toast.info("Add a new contact row directly in the table.");
  };
    ...
    <div className="p-6 space-y-6 flex flex-col h-full min-h-screen relative max-w-[1600px] mx-auto overflow-hidden">
        ...
        {selectedIds.length > 0 && (
          ...
          >
            <div className="flex items-center gap-3 px-6 border-r border-border/20 mr-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground">Contacts Selected</span>
            </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
        <div className="space-y-1">
          <div className="flex items-center gap-2 mb-2">
             <span className="px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 text-[9px] font-black uppercase tracking-[0.15em] border border-blue-500/20">Unified Directory</span>
             <div className="flex h-2 w-2 rounded-full bg-blue-500 animate-pulse" />
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-foreground uppercase font-poppins leading-none">Global <span className="text-blue-600 italic">Contacts</span></h1>
          <p className="text-muted-foreground font-bold text-xs uppercase tracking-widest opacity-60">High-density relationship management & communication synthesis.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleCreateContact} className="h-14 px-10 font-black text-[11px] uppercase tracking-[0.2em] bg-blue-600 hover:bg-blue-700 text-white rounded-2xl shadow-2xl shadow-blue-600/20 transition-all active:scale-95 group"><Plus size={20} className="mr-2 group-hover:rotate-90 transition-transform" /> Add Contact</Button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-muted/20 p-4 rounded-[2rem] border border-border/20 backdrop-blur-sm">
        <div className="relative flex-1 max-w-2xl group w-full">
          ...
          <Input placeholder="SEARCH NAMES, EMAILS, OR PHONE NUMBERS..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-14 h-14 bg-background/50 border-border/40 rounded-[1.25rem] text-[11px] font-black uppercase tracking-widest focus-visible:ring-blue-500/20 shadow-inner" />
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 border-border/40 bg-card/95 backdrop-blur-xl z-[100] rounded-2xl shadow-2xl">
              <DropdownMenuItem className="text-[10px] font-black uppercase p-4" onClick={() => { setSortBy('lastName'); setSortDirection('asc'); }}>Sort by Last Name (A-Z)</DropdownMenuItem>
              <DropdownMenuItem className="text-[10px] font-black uppercase p-4" onClick={() => { setSortBy('firstName'); setSortDirection('asc'); }}>Sort by First Name (A-Z)</DropdownMenuItem>
      ...
      <div className="flex-1 min-h-0 relative">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="h-full">
          <CRMTable 
            ...
            onEntityClick={(contact) => router.push(`/crm/contacts/${contact.id}`)}
