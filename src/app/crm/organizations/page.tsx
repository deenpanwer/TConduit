...
import { 
  ...
  Trash, Building2, Briefcase, PhoneCall, NotebookPen, UserPlus
} from "lucide-react";
 ...
 */
const CRMListView = ({ 
  organizations, config, updateConfig, onOrgClick, onOrgEdit, onAddClick, selectedIds, onSelect, onSelectAll, updateEntity, deleteEntity,
  pageSize, setPageSize, addEntity, onLaunchDeal, onLogCall, onAddNote, onAddContact
}: { 
  organizations: CRMEntity[], config: CRMConfig, updateConfig: (updates: any) => void,
  onOrgClick: (org: CRMEntity) => void, 
  onOrgEdit: (org: CRMEntity) => void,
  onAddClick: () => void,
  ...
  addEntity: (type: 'organization', data: any) => Promise<string | null>,
  onLaunchDeal: (org: CRMEntity) => void,
  onLogCall: (org: CRMEntity) => void,
  onAddNote: (org: CRMEntity) => void,
  onAddContact: (org: CRMEntity) => void
}) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const rowCount = organizations.length + tempRows.length;
      ...
      switch(e.key) {
        ...
        case 'ArrowRight':
          ...
          break;
        case 'Enter':
          const combinedRows = [...organizations, ...tempRows];
  ...
  }, [organizations, tempRows, displayFields, editingCell, editingHeaderId, focusedCell]);
  ...
  const handleCellSave = async (id: string, fieldKey: string, value: any) => {
    if (id.startsWith('temp_')) {
      const loadingToast = toast.loading("Adding new organization...");
      const newId = await addEntity('organization', { [fieldKey]: value });
      if (newId) {
  ...
  const handleAddRow = () => {
    ...
    setFocusedCell({ rowIndex: organizations.length + tempRows.length, colIndex: 0 });
  };
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-border/60 bg-card/50 overflow-hidden backdrop-blur-sm shadow-2xl overflow-x-auto relative">
        <table className="w-full text-left text-sm min-w-[1000px] border-collapse">
            ...
            <tr className="bg-secondary/40 border-b border-border/60">
              <th className="p-4 w-12 border-r border-border/60">
                <input 
                  ...
                  onChange={(e) => onSelectAll(e.target.checked ? organizations.map(o => o.id) : [])} 
                />
              </th>
          ...
          <tbody>
            {organizations.length === 0 && tempRows.length === 0 ? (
              ...
              <>
                {organizations.map((org, rowIndex) => (
                  <tr 
                    ...
                    className={cn(
                      ...
                      selectedIds.includes(org.id) && "bg-blue-500/5"
                    )}
                  >
                    ...
                    {displayFields.map((field, colIdx) => (
                      ...
                      >
                        {editingCell?.id === org.id && editingCell?.fieldId === field.id ? (
                          <InlineEdit 
                            value={String(org.data[field.key] || '')} 
                            onSave={(val) => handleCellSave(org.id, field.key, val)} 
                          ...
                          <div className="flex items-center px-4 h-full min-h-[48px]">
                            {field.type === 'select' ? (
                              <Badge variant="secondary" className={cn("text-[9px] uppercase font-black px-2 py-0 border shrink-0", `bg-${field.options?.find(o => o.value === org.data[field.key])?.color || 'gray'}-500/10 text-${field.options?.find(o => o.value === org.data[field.key])?.color || 'gray'}-500 border-${field.options?.find(o => o.value === org.data[field.key])?.color || 'gray'}-500/20`)}>
                                {field.options?.find(o => o.value === org.data[field.key])?.label || org.data[field.key] || 'None'}
                              </Badge>
                            ) : field.type === 'currency' ? (
                              <span className="font-bold text-blue-500 text-xs">${Number(org.data[field.key] || 0).toLocaleString()}</span>
                            ) : (
                              <span className={cn("text-xs truncate max-w-[200px]", field.key === 'organizationName' ? 'font-black text-foreground uppercase tracking-tight' : 'text-muted-foreground font-bold')}>
                                {String(org.data[field.key] || '-')}
                              </span>
                            )}
                            <button className="ml-auto opacity-0 group-hover/cell:opacity-100 p-1 text-muted-foreground hover:text-blue-500 transition-opacity" onClick={() => setEditingCell({ id: org.id, fieldId: field.id })}>
                              <Edit2 size={10} />
                            </button>
                          </div>
                        )}
                    ...
                    <td className="p-0 text-center w-12 border-l border-border/40">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 border-border/40 bg-card/95 backdrop-blur-xl">
                          <DropdownMenuLabel className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Org Actions</DropdownMenuLabel>
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onOrgClick(org)}><Eye size={12} className="mr-2 text-blue-500"/> View Intel</DropdownMenuItem>
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onOrgEdit(org)}><Edit2 size={12} className="mr-2 text-blue-500"/> Edit Org</DropdownMenuItem>
                          ...
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onLaunchDeal(org)}><Briefcase size={12} className="mr-2 text-green-500"/> Launch Deal</DropdownMenuItem>
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onLogCall(org)}><PhoneCall size={12} className="mr-2 text-indigo-500"/> Log Call</DropdownMenuItem>
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onAddNote(org)}><NotebookPen size={12} className="mr-2 text-orange-500"/> Add Note</DropdownMenuItem>
                          <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => onAddContact(org)}><UserPlus size={12} className="mr-2 text-purple-500"/> Add Contact</DropdownMenuItem>
                          
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="text-[10px] font-bold uppercase text-red-500" onClick={() => deleteEntity(org.id)}><Trash size={12} className="mr-2"/> Delete Org</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
                  </tr>
                ...
                {tempRows.map((row, idx) => {
                  const rowIndex = organizations.length + idx;
                  return (
              ...
              <td colSpan={displayFields.length + 3} className="p-0">
                <div className="flex items-center justify-between px-4 h-12">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground italic">
                    Showing <span className="text-foreground">{organizations.length}</span> companies in your active list
                  </p>
...
export default function OrganizationsPage() {
  const { 
    organizations, leads, config, loading, updateEntity, deleteEntity, 
    updateModuleConfig, pageSize, setPageSize, addEntity 
  } = useCRM();
  ...
  const filteredOrganizations = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(o =>
        ...
        o.data.organizationName?.toLowerCase().includes(q) ||
  ...
  const handleOpenOrg = (org: CRMEntity) => {
    setSelectedOrg(org);
    setModalMode('preview');
    setShowOrgModal(true);
  };

  const handleEditOrg = (org: CRMEntity) => {
    setSelectedOrg(org);
    setModalMode('edit');
    setShowOrgModal(true);
  };
  ...
  const handleLaunchDeal = (org: CRMEntity) => {
    setSelectedOrg(org);
    setShowDealModal(true);
  };

  const handleLogCall = (org: CRMEntity) => {
    setSelectedOrg(org);
    setShowCallModal(true);
  };

  const handleAddNote = (org: CRMEntity) => {
    setSelectedOrg(org);
    setShowNoteModal(true);
  };

  const handleAddContact = (org: CRMEntity) => {
    setSelectedOrg(org);
  ...
  return (
    <div className="p-6 space-y-6 flex flex-col h-full min-h-screen relative max-w-[1600px] mx-auto">
      ...
      <CallModal 
        ...
        mode="create"
        leads={leads}
        onSubmit={(data) => addEntity('call', data)}
        call={null}
        initialData={{
          summary: `Call regarding ${selectedOrg?.name || 'Organization'}`,
      ...
      <NoteModal 
        ...
        mode="create"
        leads={leads}
        onSubmit={(data) => addEntity('note', data)}
        note={null}
        initialData={{
          relatedTo: selectedOrg?.id,
          content: ""
      ...
      <ContactModal 
        ...
        mode="create"
        contact={null}
        initialStage={selectedOrg?.data.organizationName || selectedOrg?.name}
      />
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-foreground uppercase font-poppins">Organizations</h1>
          <p className="text-muted-foreground font-medium text-sm italic">Deep intelligence on your corporate network.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button onClick={handleCreateOrg} className="h-12 px-8 font-black text-[10px] uppercase tracking-[0.1em] bg-blue-600 hover:bg-blue-700 text-white rounded-2xl shadow-xl shadow-blue-500/20 transition-all active:scale-95 group"><Plus size={18} className="mr-2 group-hover:rotate-90 transition-transform" /> New Organization</Button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-muted/20 p-4 rounded-3xl border border-border/20">
        <div className="relative flex-1 max-w-xl group w-full">
          ...
          <Input placeholder="SEARCH ORGANIZATIONS, WEBSITES, OR INDUSTRIES..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-12 h-12 bg-background/50 border-border/40 rounded-2xl text-[10px] font-black uppercase tracking-widest focus:ring-blue-500/20" />
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 border-border bg-card/95 backdrop-blur-xl">
              <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => setSortBy("organizationName")}>Sort by Name</DropdownMenuItem>
              <DropdownMenuItem className="text-[10px] font-bold uppercase" onClick={() => setSortBy("industry")}>Sort by Industry</DropdownMenuItem>
        ...
        <AnimatePresence mode="wait">
          <motion.div key="list" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }}>
            <CRMListView 
              ...
              config={config} 
              updateConfig={(upd) => updateModuleConfig('organizations', upd)} 
              onOrgClick={handleOpenOrg} 
              onOrgEdit={handleEditOrg} 
              onAddClick={handleCreateOrg} 
