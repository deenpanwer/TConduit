...
export async function POST(request: NextRequest) {
  try {
    ...
    const { version, platform, deviceCapabilities } = body;
    ...
    await adminDb.collection('tracDiaryDownloads').add(downloadEvent);

    return NextResponse.json({ success: true, message: 'Download event logged' });
  } catch (error) {
